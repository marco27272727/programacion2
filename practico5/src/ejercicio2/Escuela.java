...
public class Escuela {
    ...
    public Escuela(String nombreDeLaCasa){
        setNombre(nombreDeLaCasa);
        casas = new ArrayList<>();
    }
    // Setters
     ...
     */
    public void agregarCasa(Casa nuevaCasa){
        if((nuevaCasa != null) && (!casas.contains(nuevaCasa))){
        casas.add(nuevaCasa);
        }
    }
    ...
    public void asignarCasaAleatoriamente(Alumno alumno){
        ...
        casaActual = casas_disponibles.remove(indice_casa);
        // Tratamos de agregar un alumno a la casa
        casaActual.agregarAlumno(alumno);
